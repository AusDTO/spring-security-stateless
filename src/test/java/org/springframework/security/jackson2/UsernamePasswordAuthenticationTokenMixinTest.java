...
public class UsernamePasswordAuthenticationTokenMixinTest extends AbstractMixinTests {
    ...
    @Test
    public void serializeAuthenticatedUsernamePasswordAuthenticationTokenMixinWithCustomUserTest() throws JsonProcessingException, JSONException {
        GrantedAuthority authority = new SimpleGrantedAuthority("ROLE_USER");
        CustomUserForTest user = new CustomUserForTest("user", "pass", "custom", Collections.singletonList(authority), true, true, true, true);
        UsernamePasswordAuthenticationToken token = new UsernamePasswordAuthenticationToken(user, "pass", Collections.singleton(authority));
        String expectedJson = "{" +
    ...
    @Test
    public void deserializeAuthenticatedUsernamePasswordAuthenticationTokenMixinWithCustomUserTest() throws Exception {
        String tokenJson = "{" +
                "  \"@class\" : \"org.springframework.security.authentication.UsernamePasswordAuthenticationToken\"," +
                "  \"details\" : null," +
                "  \"authorities\" : [ \"java.util.ArrayList\", [ {" +
        ...
        assertThat(((CustomUserForTest)token.getPrincipal()).getAuthorities()).isNotNull().hasSize(1).contains(new SimpleGrantedAuthority("ROLE_USER"));
